...
void setup() {
  ...
  pinMode(rwPin, INPUT);
  memset(m_RAM, 0, sizeof(m_RAM)); // reset RAM with 0s
  // m_RAM[65532] = 0;
  // m_RAM[65533] = 255;

  Serial.print("Apple with honey, compiled on ");
  ...
  // make sure we have clear state of the processor
  processor_reset();
  for (i = 0; i < 8; i++)
    clock_tick();
  processor_reset();
...
void loop() {
  int i, j;
  int address_dec = 0;  // address on address bus in decimal - for debugging
  char address_bin[8];  // the same address in binary - for debugging
...
//    Serial.print(" - R - ");
  ...
  read_key(); // reads keyboard
  print_char(); // prints chars in buffer - if any
} // of main loop
